...
First tagged release. The gateway has been running on mainnet in **simulation mode**
before this, deployed untagged and with no published hash; this is the first build whose
bytes are published and verifiable.
...
**Verifiability.** `scripts/release.sh` builds in a digest-pinned container, installs that
artifact, and gates on the canister reporting the hash it built.
`scripts/check-frontend-assets.py` compares every served asset against a local build.
