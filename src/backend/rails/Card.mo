...
module {
  ...
  func handleRefund(deps : Deps, refund : { eventId : Text; paymentIntent : Text }, nowNs : Int) : Http.Response {
    if (not Idempotency.recordStripeEvent(deps.dedup, refund.eventId, nowNs)) {
      return Http.text(200, "duplicate event");
    };
  ...
  func handleCheckout(deps : Deps, session : CheckoutCompleted, nowNs : Int) : Http.Response {
    ...
    if (not session.paid) {
      ...
      return Http.text(200, "ignored: payment not completed");
    };
    ...
    if (not Idempotency.recordStripeIntent(deps.dedup, session.paymentIntent, nowNs)) {
      return Http.text(200, "duplicate payment intent");
    };
    // ── Attribution (§4.1: claimed, not trusted). Failures are Type 1
    // #unattributed: fiat arrived, nothing will be minted.
    let claimedRef = switch (session.clientReferenceId) { case (?r) r; case (null) "" };
    func unattributed(detail : Text) : Http.Response {
      queueType1(deps, #unattributed({ claimedRef; paymentRef = session.paymentIntent }), detail, nowNs);
    };
    ...
    let honoredCycles = if (session.amountTotalCents == order.pricing.usdCents) {
      ...
      let ?net = Forex.netCents(order.pricing, session.amountTotalCents) else {
        return unattributed("paid amount " # session.amountTotalCents.toText() # " cents is below the fee floor of order " # orderId);
      };
      Forex.cyclesForCents(net, order.pricing.xdrPerUsdMicros);
    };
  ...
  public func handleWebhook(
    ...
    let ?key = secret else return Http.text(503, "webhook secret not provisioned");
    let ?header = Http.headerValue(req.headers, "stripe-signature") else {
      return Http.text(400, "missing Stripe-Signature header");
    };
    switch (verify(key, header, req.body, nowNs, toleranceSeconds)) {
      case (#err(_)) return Http.text(400, "signature verification failed");
      case (#ok) {};
    };
    // §4.2 opportunistic pruning: every verified delivery sweeps Stripe
    // dedup keys past the ~7-day retention.
    ignore Idempotency.pruneStripe(deps.dedup, nowNs);
    switch (parseEvent(req.body)) {
      case (#err(_)) Http.text(400, "unparseable event");
      case (#ok(#unhandled(_))) Http.text(200, "ignored");
