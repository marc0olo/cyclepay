...
#    (backend in the container; the frontend's state hash from a worktree at the tag)
...
⚠️ **Step 2 builds the frontend from the TAG, in a worktree, not from your checkout.**
The backend gets that from `git archive`; the frontend had no equivalent, and a
working-tree build would publish whatever happened to be checked out. Measured: `main`
one commit past `v0.1.0-beta.1` differs by HTML comments in `index.html`, which changes
the state hash — so the published number would have been one no verifier of that tag
could reproduce. Step 5 then confirms the canister reports the number step 2 published.
