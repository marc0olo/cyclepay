...
5. **The reproducible-build gate has never run against a real deployment** —
   `RELEASE.md`'s publish-and-verify procedure is untested end to end. ⚠️ **Measured
   2026-09-15 and worse than untested**: the container build could not succeed on any
   commit (it built the local `xrc` mock, whose wasm is not in the archive context), and
   with that fixed it produces **different bytes** than the host that deployed the live
   canister. `docs/VERIFY.md` carries the hashes and the cause.
