...
| Not covered | Why |
|---|---|
| **the reserve decision pairing a stale balance with a live tally** | ⚠️ **Verified by mutation that nothing catches it.** Replacing `Reserve.promisedForDecision` with a live-only read — the actual defect, which made `available` optimistic by a whole order at the ceiling — leaves the entire suite green. Catching it needs a delivery continuation scheduled inside `create_order`'s balance-read gap, and PocketIC gives no way to force that ordering. Scenario 72 stages the race and asserts the invariant (`promisedTotal ≤ balance`) so an over-promise arriving by another route is caught, but it is a guard rather than a proof; the arithmetic is pinned in `test/reserve.test.mo`. ⚠️ This is the one that broke a written correctness proof, so the comment at the call site is the guard |
